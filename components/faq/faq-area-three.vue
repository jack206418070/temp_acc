    ...
    <div class="main-container">
      ...
      <div class="tab-content" v-if="searchQuery == ''">
        <div class="tab-pane fade" id="nav-all" role="tabpanel" tabindex="0"
          :class="{ 'show active': activeTab === 'nav-all' }">
        ...
        <div class="tab-pane fade" id="nav-services" role="tabpanel" tabindex="0"
          ...
          <div class="accordion accordion-style-one" id="accordionTwo">
            ...
            <faq-item id="m5" title="多元陪伴照顧服務是否自費? 是否按不同試辦單位定價?"
              desc="一、 多元陪伴照顧服務使用為全額自費。  
            <br>
            二、 服務收費依試辦單位定價收費，是因試辦單位服務不同區域或服務人力， 而有不同辦理成本及有不同收費。試辦單位需於申請試辦時，提出收費標 準及計算基準經本部核定，試辦單位需依本部核定標準進行收費標準公告及收費，調整收費標準，也需要重新報請本部核定，不得任意變動。" parent="accordionTwo" />
          </div>
        </div>
...
<style scoped>
  ...
  .select::after {
    content: ''; /* 空内容，用来绘制箭头 */
      ...
      border-left: 5px solid transparent; /* 左侧透明 */
      border-right: 5px solid transparent; /* 右侧透明 */
      border-top: 5px solid #333; /* 顶部的三角形箭头 */
      pointer-events: none; /* 防止箭头影响交互 */
  }

  .select {
    position: relative; /* 确保伪元素基于 select 定位 */
  }
